            ...
            if args[0].lower() == "help":
                send_help(update.effective_chat.id, tld(chat.id, "send-help").format(
                     dispatcher.bot.first_name, "" if not ALLOW_EXCL else tld(
                         chat.id, "\nAlle Befehle können mit `/` oder `!` genutzt werden.\n"
                             )))
...
def control_panel(bot, update):
    ...
    if chat.type != chat.PRIVATE:
        ...
        update.effective_message.reply_text("Schreibe mich per PN an, um das Kontroll-System zu nutzen.",
                                            reply_markup=InlineKeyboardMarkup(
                                                [[InlineKeyboardButton(text="Systemsteuerung",
    ...
    if query:
        ...
        M_match = "Ich bin der beste :D" #LMAO, don't uncomment

    if M_match:
        text = "*Einstellungen* 🛠"

        keyboard = [[InlineKeyboardButton(text="👤 Meine Einstellungen", callback_data="cntrl_panel_U(1)")]]
        ...
        if conn:
            ...
            if member.status in ('administrator', 'creator'):
                ...
                keyboard += [[InlineKeyboardButton(text="👥 Gruppen-Einstellungen (SUDO)", callback_data="cntrl_panel_G_back")]]
            else:
                text += f"\nVerbundener Chat - *{chatG.title}* (Du bist dort kein Admin!)"
        else:
            text += "\nKein Chat verbunden!"

        keyboard += [[InlineKeyboardButton(text="Zurück", callback_data="bot_start")]]
        ...
        if mod_match:
            ...
            text = "Das sind deine aktuellen Einstellungen für das *{}* Modul:\n\n".format(
                CHAT_SETTINGS[module].__mod_name__) + R[0]

            keyboard = R[1]
            keyboard += [[InlineKeyboardButton(text="Zurück", callback_data="cntrl_panel_U(1)")]]
                
            query.message.reply_text(text=text, arse_mode=ParseMode.MARKDOWN, reply_markup=InlineKeyboardMarkup(keyboard))

        elif back_match:
            text = "*Hauptmenü der Einstellungen*"
        ...
        if not conn == False:
            ...
            query.message.reply_text(text="Chat konnte nicht verbunden werden")
            exit(1)
        ...
        if mod_match:
            ...
            text = "*{}* hat die folgenden Einstellungen für das *{}* Modul:\n\n".format(
                escape_markdown(chat.title), CHAT_SETTINGS[module].__mod_name__) + text

            keyboard += [[InlineKeyboardButton(text="Zurück", callback_data="cntrl_panel_G_back")]]
...
def help_button(bot: Bot, update: Update):
    ...
    try:
        if mod_match:
            ...
            text = tld(chat.id, "Hier ist die Hilfe für die *{}* Funktion:\n{}").format(mod_name, help_txt)
            query.message.reply_text(text=text,
                                     parse_mode=ParseMode.MARKDOWN,
                                     reply_markup=InlineKeyboardMarkup(
                                         [[InlineKeyboardButton(text=tld(chat.id, "Zurück"), callback_data="help_back")]]))
...
def get_help(bot: Bot, update: Update):
    ...
    if chat.type != chat.PRIVATE:
        ...
        update.effective_message.reply_text("Schreib' mir 'ne PN, um eine Liste mit verfügbaren Befehlen zu erhalten.",
                                            reply_markup=InlineKeyboardMarkup(
                                                [[InlineKeyboardButton(text="Hier klicken für die Hilfe!",
        ...
        text = tld(chat.id, "Hier ist die Hilfe für das *{}* Modul:\n{}").format(mod_name, help_txt)
        send_help(chat.id, text, InlineKeyboardMarkup([[InlineKeyboardButton(text=tld(chat.id, "Zurück"), callback_data="help_back")]]))
...
def send_settings(chat_id, user_id, user=False):
    if user:
        if USER_SETTINGS:
            ...
            dispatcher.bot.send_message(user_id, "Das sind deine aktuellen Einstellungen:" + "\n\n" + settings,
                                        parse_mode=ParseMode.MARKDOWN)

        else:
            dispatcher.bot.send_message(user_id, "Es sind keine Einstellungen für diesen Nutzer verfügbar :(",
                                        parse_mode=ParseMode.MARKDOWN)
        ...
        if CHAT_SETTINGS:
            ...
            dispatcher.bot.send_message(user_id,
                                        text="Für welches Modul möchtest du die Einstellungen von {} checken?".format(
                                            chat_name),
                                        reply_markup=InlineKeyboardMarkup(
                                            paginate_modules(user_id, 0, CHAT_SETTINGS, "stngs", chat=chat_id)))
        else:
            dispatcher.bot.send_message(user_id, "Es sind keine Einstellungen für diesen Chat verfügbar :(\nSende das "
                                                 "in eine Gruppe, in der du Admin bist, um ihre aktuellen Einstellungen einzusehen!",
                                        parse_mode=ParseMode.MARKDOWN)
...
def settings_button(bot: Bot, update: Update):
    ...
    try:
        if mod_match:
            ...
            chat = bot.get_chat(chat_id)
            text = "*{}* hat die folgenden Einstellungen für die *{}* Funktion:\n\n".format(escape_markdown(chat.title),
                                                                                     CHAT_SETTINGS[
                                                                                         module].__mod_name__) + \
...
def get_settings(bot: Bot, update: Update):
    ...
    if chat.type != chat.PRIVATE:
        if is_user_admin(chat, user.id):
            text = "Klicke hier, um die Einstellungen für dich, sowohl für diesen Chat einzusehen!"
            msg.reply_text(text,
                           reply_markup=InlineKeyboardMarkup(
                               [[InlineKeyboardButton(text="Einstellungen",
                                                      url="t.me/{}?start=stngs_{}".format(
                                                          bot.username, chat.id))]]))
        else:
            text = "Klicke hier, um deine Einstellungen zu checken."

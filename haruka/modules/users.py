    ...
    for chat_id in pattern.findall(message.text):
        try:
            ...
            if bot_member.can_invite_users:
                ...
                links += str(chat_id) + ":\nIch habe keinen Zugriff auf den Einladungslink." + "\n"
        except BadRequest as excp:
                links += str(chat_id) + ":\n" + excp.message + "\n"
...
def leavechat(bot: Bot, update: Update, args: List[int]):
    if args:
        ...
        update.effective_message.reply_text("Du beziehst dich auf keinen Chat!")
    try:
        chat = bot.getChat(chat_id)
        titlechat = bot.get_chat(chat_id).title
        bot.sendMessage(chat_id, "`Ich verschwinde von hier!`")
        bot.leaveChat(chat_id)
        update.effective_message.reply_text("Ich werde die Gruppe {} verlassen".format(titlechat))

    except BadRequest as excp:
        if excp.message == "Chat not found":
            update.effective_message.reply_text("Ich wurde wohl aus der Gruppe gekickt :p")
        else:
            return
...
def slist(bot: Bot, update: Update):
    message = update.effective_message
    text1 = "Meine sudos sind:"
    text2 = "Meine Support-User sind:"

...
def afk(bot: Bot, update: Update):
    ...
    update.effective_message.reply_text(tld(chat.id, f"{fname} ist nun abwesend!"))
...
def no_longer_afk(bot: Bot, update: Update):
    ...
    if res:
        ...
        try:
            update.effective_message.reply_text(tld(chat.id, f"Hey {firstname} , bist ja wieder mit uns!"))
        except:
            return
...
def check_afk(bot, update, user_id, fst_name):
    ...
    if sql.is_afk(user_id):
        ...
        if not user.reason:
            res = tld(chat.id, f"{fst_name} ist AFK!")
        else:
            res = tld(chat.id, f"{fst_name} ist AFK! Er hat den folgenden Grund angegeben:\n{user.reason}")
        update.effective_message.reply_text(res)


__help__ = """
 - /afk <Grund>: Markiert dich als abwesend
"""

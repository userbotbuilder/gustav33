...
def locktypes(bot: Bot, update: Update):
    update.effective_message.reply_text("\n - ".join(["Das kann gesperrt werden: "] + list(LOCK_TYPES) + list(RESTRICTION_TYPES)))
    ...
    if can_delete(chat, bot.id):
        if len(args) >= 1:
            if args[0] in LOCK_TYPES:
                ...
                message.reply_text(tld(chat.id, "Nachrichten, die {} enthalten, für alle Nicht-Admins gesperrt!").format(args[0]))
                ...
                message.reply_text(tld(chat.id, "Was möchtest du sperren? Schreibe /locktypes für eine Liste der möglichen Sperren"))

    else:
        message.reply_text(tld(chat.id, "Stelle sicher, dass ich Admin bin und Nachrichten löschen kann"))
    ...
    if is_user_admin(chat, message.from_user.id):
        if len(args) >= 1:
            if args[0] in LOCK_TYPES:
                ...
                message.reply_text(tld(chat.id, "Nachrichten mit {} für jeden freigeschalten!").format(args[0]))
                return "<b>{}:</b>" \
                       "\n#UNLOCK" \
                       "\n<b>Admin:</b> {}" \
                ...
                message.reply_text(tld(chat.id, "Was möchtest du entsperren? Schreibe /locktypes für eine Liste der möglichen Sperren"))

        else:
            bot.sendMessage(chat.id, tld(chat.id, "Was möchtest du entsperren?"))
                ...
                for new_mem in new_members:
                    if new_mem.is_bot:
                        if not is_bot_admin(chat, bot.id):
                            message.reply_text(tld(chat.id, "Ich sehe Bots, und ja, mir wurde befohlen, sie zu entfernen.. Eine unmögliche Aufgabe, da ich kein Admin bin!"))
                            return

                        chat.kick_member(new_mem.id)
                        message.reply_text(tld(chat.id, "Ach, noch ein Möchtegern-Admin, der sicher die Gruppe zuspammen will. Bye Bye..."))
            else:
                try:
                    message.delete()
...
def build_lock_message(chat, chatP, user, chatname):
    ...
    if not (locks or restr):
        res = tld(chatP.id, "Aktuell ist in der Gruppe *{}* nichts gesperrt.".format(chatname))
    else:
        res = tld(chatP.id, "Das sind die Sperren in der Gruppe *{}*:".format(chatname))
        if locks:
            res += "\n - sticker = `{}`" \
                   "\n - audio = `{}`" \
                   "\n - voice = `{}`" \
...
def __chat_settings__(bot, update, chat, chatP, user):
    chatname = tld(chatP.id, "dieser Chat")
...
__help__ = """
Nerven dich Sticker? Oder magst du keine Links in deiner Gruppe? oder Bilder? 
Dann bist du hier richtig!

Verfügbare Befehle:
- /locktypes: Eine Liste der verfügbaren Möglichkeiten, Nachrichten Inhalte zu sperren.

Nur für Admins:
 - /lock <Typ>: Sperrt den jegweiligen Nachrichtentyp (nicht per PN verfügbar)
 - /unlock <Typ>: Gibt den jeweiligen Nachrichtentyp wieder frei (nicht per PN verfügbar)
 - /locks: Dient der Abfrage der aktiven/inaktiven Sperren in einer Gruppe.
"""

__mod_name__ = "Inhalt sperren"

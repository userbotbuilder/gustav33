...
def extract_user_and_text(message: Message, args: List[str]) -> (Optional[int], Optional[str]):
    ...
    if entities and ent and ent.offset == len(message.text) - len(text_to_parse):
        ...
        if not user_id:
            message.reply_text("Ich kenne diesen Nutzer nicht. Mach ihn mir bekannt, indem  "
                               "du mir eine Nachricht von ihm weiterleitest, und auf sie antwortest.")
            return None, None
    ...
    try:
        ...
        if excp.message in ("User_id_invalid", "Chat not found"):
            message.reply_text("Ich kenne diesen Nutzer nicht. Mach ihn mir bekannt, indem "
                               "du mir eine Nachricht von ihm weiterleitest, und auf sie antwortest. "
                               "dann werden wir evtl beste Freunde^^")
        else:

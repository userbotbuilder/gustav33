...
def bot_can_delete(func):
    ...
    def delete_rights(bot: Bot, update: Update, *args, **kwargs):
        if can_delete(update.effective_chat, bot.id):
            ...
            update.effective_message.reply_text("Ich kann hier keine Nachrichten löschen! "
                                                "Stelle sicher, dass ich admin bin und Nachrichten löschen kann")
...
def can_pin(func):
    ...
    def pin_rights(bot: Bot, update: Update, *args, **kwargs):
        if update.effective_chat.get_member(bot.id).can_pin_messages:
            ...
            update.effective_message.reply_text("Ich kann hier keine Nachrichten anpinnen! "
                                                "Stelle sicher, dass ich Admin bin, und nchrichten anpinnen kann.")
...
def can_promote(func):
    ...
    def promote_rights(bot: Bot, update: Update, *args, **kwargs):
        if update.effective_chat.get_member(bot.id).can_promote_members:
            ...
            update.effective_message.reply_text("Ich kann hier keine Leute zum Admin machen/Degradieren! "
                                                "Stelle sicher, dass ich Admin bin, und neue Admins hinzufügen kann.")
...
def can_restrict(func):
    ...
    def promote_rights(bot: Bot, update: Update, *args, **kwargs):
        if update.effective_chat.get_member(bot.id).can_restrict_members:
            ...
            update.effective_message.reply_text("Ich kann hier keine Personen einschränken! "
                                                "Stelle sicher, dass ich Admin bin, und neue Admins hinzufügen kann.")
...
def bot_admin(func):
    ...
    def is_admin(bot: Bot, update: Update, *args, **kwargs):
        if is_bot_admin(update.effective_chat, bot.id):
            ...
            update.effective_message.reply_text("Ich bin kein Admin!")
...
def user_admin(func):
    ...
    def is_admin(bot: Bot, update: Update, *args, **kwargs):
        ...
        if user and is_user_admin(update.effective_chat, user.id):
            ...
            update.effective_message.reply_text("Ach, du nicht-Admin willst mir zeigen, wo's lang geht?")

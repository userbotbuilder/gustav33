...
DEFAULT_WELCOME = "Hey {first}, Herzlich Willkommen!✌"
DEFAULT_GOODBYE = "Mach's gut, {fullname}!"
...
class Welcome(BASE):
    ...
    def __repr__(self):
        return "<Chat {} sollte neue Nutzer willkommen heißen: {}>".format(self.chat_id, self.should_welcome)
...
class CleanServiceSetting(BASE):
    ...
    def __repr__(self):
        return "<Chat hat clean service genutzt ({})>".format(self.chat_id)
...
def welcome_security(chat_id):
    try:
        ...
        if security:
            ...
            return False, "0", "Ich bin kein Bot!"
    finally:
        SESSION.close()

...
def add_blacklist_url(bot: Bot, update: Update):
    ...
    if len(urls) > 1:
        ...
        if len(to_blacklist) == 1:
            ...
            if extract_url.domain and extract_url.suffix:
                message.reply_text(
                    "Die Domain <code>{}</code> steht nun auf der Blacklist!".format(
                        html.escape(
                            extract_url.domain + "." + extract_url.suffix)),
                    parse_mode=ParseMode.HTML)
            else:
                message.reply_text(
                    "Du willst eine ungültige URL zur Blacklist hinzufügen!")
        else:
            message.reply_text(
                "Die Domains <code>{}</code> stehen nun auf der Blacklist.".format(
                    len(blacklisted)), parse_mode=ParseMode.HTML)
    else:
        message.reply_text(
            "Welche URL möchtest du zu der Blacklist hinzufügen?")
...
def rm_blacklist_url(bot: Bot, update: Update):
    ...
    if len(urls) > 1:
        ...
        if len(to_unblacklist) == 1:
            if unblacklisted:
                message.reply_text(
                    "Ich habe <code>{}</code> von der Blacklist gestrichen!".format(
                        html.escape(
                            to_unblacklist[0])),
                    parse_mode=ParseMode.HTML)
            else:
                message.reply_text("Das ist keine Domain, die auf der Blacklist steht..!")
        elif unblacklisted == len(to_unblacklist):
            message.reply_text(
                "Ich habe <code>{}</code> Domains von der Blacklist gestrichen!".format(
                    unblacklisted), parse_mode=ParseMode.HTML)
        elif not unblacklisted:
            message.reply_text(
                "Keine dieser URLs existiert, also habe ich keine entfernt.",
                parse_mode=ParseMode.HTML)
        else:
            message.reply_text(
                "Ich habe <code>{}</code> Domains von der Blacklist entfernt. {} existierten nicht, "
                "also habe ich die nicht entfernt.".format(
                    unblacklisted,
                    len(to_unblacklist) - unblacklisted),
                parse_mode=ParseMode.HTML)
    else:
        message.reply_text(
            "Sag mir, welchen Blacklist-Eintrag du entfernen möchtest.")
        ...
        if url in extracted_domains:
            try:
                ...
                if excp.message == "Message to delete not found":
                    ...
                    LOGGER.exception("Fehler während des Streichens dieses Blacklist-Eintrags.")
            break
...
def get_blacklisted_urls(bot: Bot, update: Update):
    ...
    base_string = "Aktuell <b>geblacklistete</b> Domains:\n"
    blacklisted = sql.get_blacklisted_urls(chat.id)

    if not blacklisted:
        message.reply_text("Hier stehen keine Domains auf der Blacklist!")
        return
...
__help__ = """
Domain-Blacklisting erlaubt es, festgelegte Domains zu blocken,
sodass eine Nachricht, die diese Domain beinhaltet, sofort gelöscht wird.

*INFO:* Geblockte Domains sind für Admins freigegeben.

- /geturl: Die aktuellen Einträge auf der Blacklist anzeigen lassen

*Nur für Admins:*

- /addurl <URLs>: Eine/Mehrere URLs/Domains der Blacklist hinzufügen.
- /delurl <URLs>: URLs/Domains aus der Blacklist streichen

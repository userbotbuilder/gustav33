...
def gban(bot: Bot, update: Update, args: List[str]):
    ...
    if not user_id:
        message.reply_text("Stop mal.. Wen meinst du?")
        return

    if int(user_id) in SUDO_USERS:
        message.reply_text("Wenn die Sudos sich streiten haha")
        return

    if int(user_id) in SUPPORT_USERS:
        message.reply_text("OOOH jemand will nen Sudo user Gbannen! *Popcorn kommt*")
        return

    if user_id == bot.id:
        message.reply_text("-_- Ja, ich verbanne mich nun überall, wo ich drin bin... Netter Versuch")
        return
    ...
    if user_chat.first_name == '':
        message.reply_text("Das ist ein gelöschtes Konto! Wieso sollte ich es global bannen?")
        return

    if sql.is_user_gbanned(user_id):
        if not reason:
            message.reply_text("Dieser Nutzer ist bereits Gbanned.")
            return
        ...
        if old_reason:
            ...
            new_reason = f"{new_reason} // GBanned von {bannername} id {bannerid}"
            ...
            message.reply_text("Dieser Nutzer ist bereits Gbanned, aus folgendem Grund:\n"
                               "<code>{}</code>\n"
                               "Ich habe den Grund aktualisiert!".format(html.escape(old_reason)),
                               parse_mode=ParseMode.HTML)
        else:
            banner = update.effective_user  # type: Optional[User]
            bannerid = banner.id
            ...
            message.reply_text("Ich habe den Grund für den Gban dieses Nutzers notiert!")
...
def ungban(bot: Bot, update: Update, args: List[str]):
    ...
    if not user_id:
        message.reply_text("Sorry.. Wen meinst du?")
        return
    ...
    if not sql.is_user_gbanned(user_id):
        message.reply_text("Dieser Nutzer ist nicht Gbannt!")
        return

    banner = update.effective_user  # type: Optional[User]

    message.reply_text("Ich gebe dem lieben {} 'ne zweite chance, global.".format(user_chat.first_name))

    bot.send_message(MESSAGE_DUMP,
                 "{} hat {} global entbannt".format(mention_html(banner.id, banner.first_name),
                                                   mention_html(user_chat.id, user_chat.first_name)),
                 parse_mode=ParseMode.HTML)
...
def antispam(bot: Bot, update: Update, args: List[str]):
    ...
    if len(args) > 0:
        if args[0].lower() in ["on", "yes"]:
            ...
            update.effective_message.reply_text(tld(chat.id, "Ich habe Antispam in dieser Gruppe aktiviert. Nun werde ich diese Gruppe vor bekannten Spammern, "
                                                "ungewünschten Persönlichkeiten und den größten Trolls schützen."))
        elif args[0].lower() in ["off", "no"]:
            sql.disable_antispam(chat.id)
            update.effective_message.reply_text(tld(chat.id, "Ich habe die Antispam-Funktion hier deaktiviert. Somit ist die  "
                                                "Gruppe nicht mehr vor bekannten Störer*innen "
                                                "geschützt."))
    else:
        update.effective_message.reply_text(tld(chat.id, "Gib' mir auch den Zustand deiner gewünschten Einstellung! on/off, yes/no!\n\n"
                                            "Deine aktuelle Einstellung ist: {}\n"
                                            "Falls true, ist die Gruppe geschützt. "
                                            "Falls der Zustand false beträgt, ist deine Gruppe vor ungewünschten Personen "
                                            "ausgesetzt.").format(sql.does_chat_gban(chat.id)))
...
def __chat_settings__(bot, update, chat, chatP, user):
    chat_id = chat.id
    return "Der Zustand für *gbans* in dieser Gruppe: `{}`.".format(sql.does_chat_gban(chat_id))


__help__ = """
*Nur für Admins:*
 - /antispam <on/off/yes/no>: Schält Antispam ein oder aus. 
"""

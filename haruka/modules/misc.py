    ...
    if user_id:
        if update.effective_message.reply_to_message and update.effective_message.reply_to_message.forward_from:
            ...
            update.effective_message.reply_text(tld(chat.id,
                "Der ursprüngliche Sender ({}) hat die ID `{}`.\nDer Weiterleiter ({}) hat die ID `{}`.").format(
            ...
            update.effective_message.reply_text(tld(chat.id, " Die ID von {} ist `{}`.").format(escape_markdown(user.first_name), user.id),
                                                parse_mode=ParseMode.MARKDOWN)
    else:
        chat = update.effective_chat  # type: Optional[Chat]
        if chat.type == "private":
            update.effective_message.reply_text(tld(chat.id, "Deine ID ist `{}`.").format(chat.id),
                                                parse_mode=ParseMode.MARKDOWN)

        else:
            update.effective_message.reply_text(tld(chat.id, "Die ID dieser Gruppe ist `{}`.").format(chat.id),
                                                parse_mode=ParseMode.MARKDOWN)
...
def info(bot: Bot, update: Update, args: List[str]):
    ...
    if user_id:
        ...
        msg.reply_text(tld(chat.id, "Daraus kann ich keinen Nutzer auslesen."))
        return

    else:
        return

    text =  tld(chat.id, "<b>Nutzer-Infos</b>:")
    text += "\nID: <code>{}</code>".format(user.id)
    text += tld(chat.id, "\nVorname: {}").format(html.escape(user.first_name))

    if user.last_name:
        text += tld(chat.id, "\nNachname: {}").format(html.escape(user.last_name))

    if user.username:
        text += tld(chat.id, "\nBenutzername: @{}").format(html.escape(user.username))
    ...
    if user.id == OWNER_ID:
        ...
        if user.id in SUDO_USERS:
            text += tld(chat.id, "\nDiese Person ist eine meiner Sudos! " \
            "Fast so Mächtig wie mein Inhaber- Obacht!")
        else:
...
def lyrics(bot: Bot, update: Update, args: List[str]):
    ...
    if len(song) == 2:
        ...
        try:
            ...
            return update.effective_message.reply_text("Song %s nicht gefunden :(" % song[1], failed=True)
        else:
...
def paste(bot: Bot, update: Update, args: List[str]):
    ...
    if message.reply_to_message:
        ...
        message.reply_text("Was soll ich damit anfangen?!")
        return

    r = requests.post(f'{BASE_URL}/documents', data=data.encode('utf-8'))

    if r.status_code == 404:
        update.effective_message.reply_text('Dogbin ist nicht erreichbar')
        r.raise_for_status()
    ...
    key = res['key']
    if res['isUrl']:
        reply = f'Gekürzte URL: {BASE_URL}/{key}\nDu kannst Statistiken etc... [hier einsehen]({BASE_URL}/v/{key})'
    else:
...
def get_paste_content(bot: Bot, update: Update, args: List[str]):
    ...
    if len(args) >= 1:
        ...
        message.reply_text("Gib' bitte nen Paste-Key an!")
        return
...
def get_paste_stats(bot: Bot, update: Update, args: List[str]):
    ...
    if len(args) >= 1:
        ...
        message.reply_text("Gib bitte nen Pastekey an!")
        return
    ...
    reply = f'Statistiken für **[/{key}]({BASE_URL}/{key})**:\nSo oft abgerufen: `{views}`'
...
def wiki(bot: Bot, update: Update):
    ...
    if len(str(kueri[1])) == 0:
        ...
        try:
            pertama = update.effective_message.reply_text("🔄 Moment...")
            keyboard = InlineKeyboardMarkup([[InlineKeyboardButton(text="🔧 Mehr Info", url=wikipedia.page(kueri).url)]])
            ...
            update.effective_message.reply_text(f"⚠ Fehler: {e}")
        except BadRequest as et :
            update.effective_message.reply_text(f"⚠ Fehler: {et}")
        except wikipedia.exceptions.DisambiguationError as eet:
            update.effective_message.reply_text(f"⚠ Error\n There are too many query! Express it more!\nPossible query result:\n{eet}")


__help__ = """
 - /id: Schickt die aktuelle Gruppen-ID. Wenn sie beim Antworten auf eine Nachricht verwendet wird, wird die ID des Benutzers abgerufen.
 - /info: Info über einen Nutzer erhalten.
 - /getsticker: Auf einen Sticker antworten, um ihn als .png zu erhalten!
 - /lyrics: Finde die Lyrics deines Lieblingssongs!
 - /paste: Create a paste or a shortened url using [dogbin](https://del.dog)
 - /getpaste: Get the content of a paste or shortened url from [dogbin](https://del.dog)
 - /pastestats: Statistiken einer gekürzten URL abrufen
 - /ud: Wort angeben, nachdem gesucht werden soll. z.B. /ud ShityBrain
 - /exec <Programmiersprache> <code> [/stdin <stdin>]: Einen Code in der gewünschten Programmiersprache ausführen
"""

__mod_name__ = "Gemischtes"
